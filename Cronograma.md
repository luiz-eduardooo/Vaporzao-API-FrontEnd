# 📅 Cronograma — Vaporzão Frontend

Cronograma de entregas do projeto, conforme definido pelo professor.

**Legenda:** ✅ concluído · 🔄 em andamento · ⬜ pendente

---

## 🟢 Segunda entrega — 26/05 (terça-feira)

### Desenvolvimento

- ⬜ Estruturação de pastas
- ⬜ Conexão de API
- ⬜ Componentização — cards de "lançamentos recentes", "mais bem avaliados"
  e "populares na comunidade"
- ⬜ Desenvolvimento do cabeçalho com botões *(sem redirecionamento)*
- ⬜ Desenvolvimento do rodapé *(links sem redirecionamento)*

---

## 🟡 Terceira entrega — 29/05 (sexta-feira)

### Desenvolvimento II

- ⬜ Barra de pesquisa e ícones (carrinho, perfil e notificações) ¹
- ⬜ Filtros — navegar por gênero
- ⬜ Página para adicionar/criar jogo
- ⬜ Criar base de dados, com imagens
- ⬜ Carrossel dos jogos em destaque

---

## 🟠 Quarta entrega — 02/06 (terça-feira)

### Desenvolvimento III

- ⬜ Tela da biblioteca
- ⬜ Tela da wishlist
- ⬜ Pesquisa funcionando
- ⬜ Botão de perfil funcionando
- ⬜ Ao clicar no card, abrir tela de detalhes
- ⬜ Tela da criação de jogos

---

## 🔴 Entrega final — 09/06 (terça-feira)

### Desenvolvimento final

- ⬜ Tela de login e cadastro
- ⬜ Refinamento em detalhes visuais
- ⬜ Explicação de uso de IA

---

## 📝 Notas de adaptação

**¹ Ícones de carrinho e notificações:** a Vaporzão API não possui endpoints
de carrinho de compras nem de notificações. Para cumprir o item do cronograma,
esses ícones serão implementados no cabeçalho como elementos **decorativos /
não funcionais** (exibindo um aviso de "em breve" ao clicar), o que está
alinhado com a definição da segunda entrega de que os botões do cabeçalho
"não redirecionam". As funcionalidades reais de coleção de jogos são atendidas
pelas telas de **Biblioteca** e **Wishlist**.